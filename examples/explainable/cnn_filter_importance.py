    ...
    for i in range(0, data.shape[0], batch_size):
        ...
        if not intermediate_output:
            ...
            for j in range(len(intermediate_output)):
                intermediate_output[j] = np.concatenate(
                    (intermediate_output[j], batch_output[j]), axis=0
                )
...
train_transformed = [
    frobenius_norm(layer_response) for layer_response in train_responses
]
...
for layer, filter_index, importance in filter_importances[
    :10
]:  # Display top 10 important filters
