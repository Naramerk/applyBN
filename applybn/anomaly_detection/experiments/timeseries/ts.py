...
from applybn.anomaly_detection.scores.proximity_based import (
    LocalOutlierScore,
...
def get_creditcard():
    df1 = pd.read_csv("../../data/ts/creditcard_p1.csv", sep=",", index_col=0).drop(
        ["Amount", "Time"], axis=1
    )
    df2 = pd.read_csv("../../data/ts/creditcard_p2.csv", sep=",", index_col=0).drop(
        ["Amount", "Time"], axis=1
    )
...
def body(df_getter, scorer_class, cv=10, additional_scorer=None):
    ...
    match scorer_class.__name__:
        ...
        case "LocalOutlierScore" | "IsolationForestScore":
            ...
            scorer = scorer_class()
        case "ODBPScore":
            df, y, estimator, encoding = learn_structure(
                df_getter, return_encoding=True
            )
    ...
    for train_indexes, test_indexes in splitter.split(df, y):
        ...
        detector = TabularDetector(estimator, score=scorer, target_name=None)
...
def learn_structure(df_getter: callable, full=False, return_encoding=False):
    ...
    estimator = BNEstimator(has_logit=True, use_mixture=False, bn_type=bn_type)
    ...
    estimator.fit(
        discretized_data, clean_data=df, partial=True, descriptor=info, **bn_params
    )  # , progress_bar=False
...
for params in grid:
    ...
    print(
        body(
            params["df_getter_args"],
            scorer_class=params["score"],
            additional_scorer=LocalOutlierScore(),
        )
    )
