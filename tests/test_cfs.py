import pytest
import numpy as np
from applybn.feature_selection.ce_feature_selector import (
    CausalFeatureSelector,
)  # Предполагается, что класс сохранен в файле `causal_feature_selector.py`
...
def test_discretize_data_iqr():
    ...
    selector = CausalFeatureSelector(
        n_bins=3
    )  # Initialize with a specific number of bins
